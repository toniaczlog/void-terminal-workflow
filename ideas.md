...
Ostatnia aktualizacja statusów: 2026-09-02 — wdrożono 14 z 20 pozycji.

---

## Tabela zbiorcza

| ID | Pomysł | Cel | Koszt | Status |
|---|---|---|---|---|
| UI-01 | `scroll-mt` na kotwicach sekcji | Fixed navbar zasłania nagłówki po kliknięciu w link kotwicy | 1 | ✅ wdrożone |
| UI-02 | Sticky mini-nav w sekcji CONTEXT.md | Skok do konkretnej komendy w sekcji o wysokości ~6400 px | 4 | ✅ wdrożone |
| UI-03 | Meta description + OG/Twitter + favicon | Poprawny podgląd linku w mediach społecznościowych | 2 | ⬜ do zrobienia |
| UI-04 | Przycisk „Pobierz .ps1" w generatorze | Gotowy plik zamiast kopiowania 44 KB przez schowek | 2 | ✅ wdrożone |
| UI-05 | Aktualizacja `docs.html` o komendy `ctx-*` | Cheatsheet zgodny z faktyczną zawartością profilu | 3 | ✅ wdrożone |
| UI-06 | Live-filtr komend na `docs.html` | Znalezienie komendy wśród ~30 pozycji w sekundę | 4 | ✅ wdrożone |
| UI-07 | Dostępność (reduced-motion, focus, skip-link) | Obsługa klawiaturą i wyłączenie animacji dla wrażliwych | 3 | ✅ wdrożone |
| LOG-01 | Dopisać brakującą funkcję `scratch` | Komenda reklamowana w dokumentacji, ale nieistniejąca w kodzie | 2 | ✅ wdrożone |
| LOG-02 | Fallback generatora bez `fetch()` | Generator działa też po otwarciu `index.html` z dysku | 4 | ✅ wdrożone |
| LOG-03 | `init-ctx nextjs` / `init-ctx vps` | Wykorzystanie leżących odłogiem szablonów kontekstu | 3 | ✅ wdrożone |
| LOG-04 | `radar`: `Push-Location`/`Pop-Location` | Brak ryzyka pozostawienia użytkownika w cudzym katalogu | 2 | ✅ wdrożone |
| LOG-05 | `todo rm` po indeksie zamiast `IndexOf` | Poprawne usuwanie przy zdublowanych nazwach zadań | 2 | ✅ wdrożone |
| LOG-06 | Komenda `handoff` | Domknięcie pętli pamięci: sesja → podsumowanie → CONTEXT.md | 4 | ⬜ do zrobienia |
| LOG-07 | `void doctor` — audyt środowiska | Wykrycie braków (git, gh, node, code) przed pierwszym błędem | 3 | ✅ wdrożone |
| LOG-08 | Naprawa README | Usunięcie odwołań do nieistniejących plików, aktualna roadmapa | 3 | ⬜ do zrobienia |
| OPT-01 | Cache danych HUD (CPU, bateria, dysk) | Odczuwalnie szybszy znak zachęty po każdej komendzie | 5 | ✅ wdrożone |
| OPT-02 | `Get-Clipboard` poza prompt | Koniec odczytu schowka po każdej komendzie (koszt + prywatność) | 1 | ✅ wdrożone |
| OPT-03 | Landing: lazy xterm + fonty przez `<link>` | Krótszy czas do pierwszego renderu strony głównej | 4 | ⬜ do zrobienia |
| OPT-04 | Porządki w repo | Usunięcie `test.js`, dodanie `.gitignore`, `master` → `main` | 2 | ⬜ do zrobienia |
| OPT-05 | CI: parse + PSScriptAnalyzer | Złamana składnia profilu nie trafia do użytkowników | 4 | ⬜ do zrobienia |

---

## UI

### UI-01 — `scroll-mt` na kotwicach sekcji · koszt 1 · ✅ wdrożone
...
### UI-02 — Sticky mini-nav w sekcji CONTEXT.md · koszt 4 · ✅ wdrożone
...
### UI-04 — Przycisk „Pobierz .ps1" w generatorze · koszt 2 · ✅ wdrożone
...
### UI-05 — Aktualizacja `docs.html` o komendy `ctx-*` · koszt 3 · ✅ wdrożone
...
### UI-06 — Live-filtr komend na `docs.html` · koszt 4 · ✅ wdrożone
...
### UI-07 — Dostępność · koszt 3 · ✅ wdrożone
...
### LOG-01 — Dopisać brakującą funkcję `scratch` · koszt 2 · ✅ wdrożone
...
### LOG-02 — Fallback generatora bez `fetch()` · koszt 4 · ✅ wdrożone
...
### LOG-03 — `init-ctx nextjs` / `init-ctx vps` · koszt 3 · ✅ wdrożone
...
### LOG-04 — `radar`: `Push-Location`/`Pop-Location` · koszt 2 · ✅ wdrożone
...
### LOG-05 — `todo rm` po indeksie zamiast `IndexOf` · koszt 2 · ✅ wdrożone
...
### LOG-07 — `void doctor` — audyt środowiska · koszt 3 · ✅ wdrożone
...
### OPT-01 — Cache danych HUD · koszt 5 · ✅ wdrożone
...
### OPT-02 — `Get-Clipboard` poza prompt · koszt 1 · ✅ wdrożone
